    ...
    match dir_data_result {
        ...
        Ok(dir_data) => {
            ...
            for cat_entry in &catalog.entries {
                let header_info = match file_has_amsdos_header(&dsk, &dir_data, &cat_entry.name) {
                    Some(true) => "AMSDOS header valid",
                    Some(false) => "no AMSDOS header (ASCII/raw)",
                    None => "cannot read first block",
                };
                println!(
                    "  {} {} \u{2014} {}",
                    "✓".bright_green(),
