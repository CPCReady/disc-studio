...
impl Dsk {
    ...
    fn get_sector_position(&self, track: usize, sector_id: usize) -> Result<usize> {
        // Navigate to the correct track using per-track sizes (EXTENDED) or fixed size (standard).
        let mut track_info_pos = DskHeader::SIZE;
        for t in 0..track {
            let ts = self.header.track_size_for(t);
            if ts == 0 {
                return Err(DskError::InvalidFormat(format!(
                    "Track {} is unformatted or has unknown size",
                    t
                )));
            }
            track_info_pos += ts;
        ...
        // Sector data immediately follows the 256-byte TrackInfo block.
        let sector_data_start = track_info_pos + TrackInfo::SIZE;

        if track_info_pos + 22 > self.data.len() {
            return Err(DskError::InvalidFormat(format!(
                "Track {} info out of bounds",
                track
            )));
        }
        ...
        // Search for a sector whose ID (R field) matches the requested sector_id.
        for s in 0..num_sectors as usize {
            ...
            if sector_info_pos + 2 < self.data.len() {
                ...
                if sid as usize == sector_id {
                    return Ok(sector_data_start + s * SECTOR_SIZE);
                }
            }
        }

        // Sector ID not found — use physical (sequential) position as fallback.
        Ok(sector_data_start + sector_id * SECTOR_SIZE)
    }
