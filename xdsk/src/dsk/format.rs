...
pub struct DskHeader {
    ...
    pub tracks: u8,
    pub heads: u8,
    /// Fixed track size for standard (MV) DSK. Always 0 for EXTENDED format.
    pub track_size: u16,
    /// Per-track sizes for EXTENDED DSK format (byte at offset 52+i, multiplied by 256).
    /// Empty for standard DSK.
    pub track_sizes: Vec<u16>,
}
...
impl DskHeader {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        ...
        // For EXTENDED CPC DSK, parse per-track size table at offset 52.
        // Each byte encodes: actual_track_size = byte * 256.
        // For multi-sided disks the table interleaves: t0h0, t0h1, t1h0, t1h1...
        let mut track_sizes = Vec::new();
        if magic.starts_with("EXTENDED CPC DSK") {
            let total_entries = tracks as usize * heads.max(1) as usize;
            for i in 0..total_entries {
                let offset = 52 + i;
                track_sizes.push(if offset < data.len() {
                    data[offset] as u16 * 256
                } else {
                    0
                });
            }
    ...
    /// Returns true if this is an EXTENDED CPC DSK image.
    pub fn is_extended(&self) -> bool {
        self.magic.starts_with("EXTENDED CPC DSK")
    }

    /// Returns the size (in bytes) of the given track's block (TrackInfo + sector data).
    /// For standard DSK all tracks have the same fixed size.
    /// For EXTENDED DSK each track has its own size stored in the header table.
    pub fn track_size_for(&self, track: usize) -> usize {
        if self.is_extended() {
            let idx = track * self.heads.max(1) as usize;
            if idx < self.track_sizes.len() {
                self.track_sizes[idx] as usize
            } else {
                0
            }
        } else {
            self.track_size as usize
        }
    }
}
