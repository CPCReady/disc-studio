    ...
    pub fn from_dsk(dsk: &Dsk) -> Result<Self> {
        ...
        for i in 0..MAX_DIR_ENTRIES {
            ...
            if !dir_entry.is_deleted() && dir_entry.is_first_extent() {
                ...
                let (file_type, load_address, exec_address) = detect_file_type(dsk, &dir_entry);
...
fn detect_file_type(dsk: &Dsk, dir_entry: &DirEntry) -> (String, Option<u16>, Option<u16>) {
    // Read first block to check for AMSDOS header
    if dir_entry.blocks[0] == 0 {
        return ("ASCII".to_string(), None, None);
    }
    ...
    if let Ok(block) = dsk.read_block(dir_entry.blocks[0]) {
        ...
        if crate::amsdos::has_header(&block) {
            ...
            // load_address at offset 21-22 (LE), entry_address at offset 26-27 (LE)
            let load_address = Some(u16::from_le_bytes([block[21], block[22]]));
            let exec_address = Some(u16::from_le_bytes([block[26], block[27]]));
