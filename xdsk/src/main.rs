...
fn main() -> Result<()> {
    ...
    let result = match cli.command {
        ...
        Commands::Chuser { image, file, user } =>
            commands::chuser::execute(&image, &file, user),
        Commands::Attr {
            image,
            file,
            read_only,
            ...
            system,
            no_system,
        } => commands::attr::execute(
            &image,
            &file,
            commands::attr::AttrOptions {
                read_only: if read_only { Some(true) } else if no_read_only { Some(false) } else { None },
                system: if system { Some(true) } else if no_system { Some(false) } else { None },
            },
        ),
        ...
        Commands::Completions { shell } => {
            generate(shell, &mut Cli::command(), "xdsk", &mut std::io::stdout());
            return Ok(());
        }
...
fn generate_man_pages(output_dir: &Path) -> Result<()> {
    ...
    let main_path = output_dir.join("xdsk.1");
    ...
    for sub in cmd.get_subcommands() {
        ...
        Man::new(sub.clone())
            .render(&mut buf)
            .with_context(|| format!("Failed to render man page for xdsk-{}", sub_name))?;

        // File is stored as xdsk-<subcommand>.1 so man(1) finds it as "man xdsk-list", etc.
        let out_path = output_dir.join(format!("xdsk-{}.1", sub_name));
