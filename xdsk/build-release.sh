...
artifact_name() {
  local target="$1"
  case "$target" in
    x86_64-unknown-linux-gnu)   echo "xdsk-linux-x86_64" ;;
    aarch64-unknown-linux-gnu)  echo "xdsk-linux-aarch64" ;;
    x86_64-apple-darwin)        echo "xdsk-macos-x86_64" ;;
    aarch64-apple-darwin)       echo "xdsk-macos-aarch64" ;;
    x86_64-pc-windows-msvc)     echo "xdsk-windows-x86_64.exe" ;;
    *) echo "xdsk-$target" ;;
  esac
}
...
binary_path() {
  ...
  if [[ "$target" == *windows* ]]; then
    echo "target/$target/release/xdsk.exe"
  else
    echo "target/$target/release/xdsk"
  fi
}
